                ...
                <div class="p-1 pt-4 p-md-5">
                    ...
                    <form class="user" action="{{ route('admin##changePassword') }} " method="post">
                        ...
                        <div class="form-group">
                            <input type="password" name="oldPassword" placeholder="Enter Old Password"
                                class="form-control form-control-user @error('oldPassword') is-invalid @enderror"
                        ...
                        <div class="form-group">
                            <input type="password" name="newPassword" placeholder="Enter New Password"
                                class="form-control form-control-user @error('newPassword') is-invalid  @enderror"
                                id="exampleInputPassword" value="{{ old('newPassword') }}">
                            @error('newPassword')
                    ...
                    <div class="text-center ">
                        @if (session('notMatched'))
                           <span class="text-danger"> {{ session('notMatched') }}</span>
                           <a class="small" href="register.html">Try another way?</a>
                        @endif
                        @if (session('matched'))
                    <div class="alert alert-info alert-dismissible fade show">
                        <h4 class="text-success"><i class="fa-solid fa-shield-halved"></i> {{ session('matched') }}
                        </h4>
